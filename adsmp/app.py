...
from adsmsg import (
    ...
    BoostRequestRecord,
    BoostRequestRecordList,
    BoostResponseRecord,
    BoostResponseRecordList,
...
from adsmsg import Status as AdsMsgStatus
from adsmsg.msg import Msg
from adsputils import (
    ADSCelery,
    contextmanager,
    create_engine,
    scoped_session,
...
from sqlalchemy import Table, bindparam, exc, func
from sqlalchemy.dialects.postgresql import insert
from sqlalchemy.orm import load_only as _load_only

from adsmp import solr_updater, templates
from adsmp.models import (
    ChangeLog,
    IdentifierMapping,
    MetricsBase,
    MetricsModel,
...
class ADSMasterPipelineCelery(ADSCelery):
    def __init__(self, app_name, *args, **kwargs):
        ...
        if self._config.get("METRICS_SQLALCHEMY_URL", None):
            ...
            insert_columns = {
                "an_refereed_citations": bindparam(
                    "an_refereed_citations", required=False
                ),
                ...
                "refereed_citation_num": bindparam(
                    "refereed_citation_num", required=False
                ),
            ...
            update_columns = {
                "an_refereed_citations": getattr(
                    self._metrics_table_upsert.excluded, "an_refereed_citations"
                ),
                "an_citations": getattr(
                    self._metrics_table_upsert.excluded, "an_citations"
                ),
                "author_num": getattr(
                    self._metrics_table_upsert.excluded, "author_num"
                ),
                "citations": getattr(self._metrics_table_upsert.excluded, "citations"),
                "citation_num": getattr(
                    self._metrics_table_upsert.excluded, "citation_num"
                ),
                ...
                "refereed_citations": getattr(
                    self._metrics_table_upsert.excluded, "refereed_citations"
                ),
                "refereed_citation_num": getattr(
                    self._metrics_table_upsert.excluded, "refereed_citation_num"
                ),
                "reference_num": getattr(
                    self._metrics_table_upsert.excluded, "reference_num"
                ),
                "rn_citations": getattr(
                    self._metrics_table_upsert.excluded, "rn_citations"
                ),
                "rn_citation_data": getattr(
                    self._metrics_table_upsert.excluded, "rn_citation_data"
                ),
            }
            self._metrics_table_upsert = (
                self._metrics_table_upsert.on_conflict_do_update(
        ...
        with self.session_scope() as session:
            ...
            try:
                ...
                if not record.scix_id and record.bib_data:
                    attempted_scix_id = "scix:" + str(
                        self.generate_scix_id(record.bib_data)
                    )
                ...
                if attempted_scix_id:
                    self.log_scix_id_collision(bibcode, attempted_scix_id)
                else:
                    self.logger.exception(
                        "IntegrityError in update_storage for bibcode %s, type %s "
                        "(not a scix_id collision)",
                        bibcode,
                        type,
                    )
                raise
    ...
    def log_scix_id_collision(self, bibcode, attempted_scix_id):
        ...
        try:
            with self.session_scope() as session:
                existing = (
                    session.query(Records).filter_by(scix_id=attempted_scix_id).first()
                )
                if existing:
                    self.logger.error(
    ...
    def delete_by_bibcode(self, bibcode):
        with self.session_scope() as session:
            ...
            if r is not None:
                ...
                session.query(SitemapInfo).filter_by(bibcode=bibcode).delete(
                    synchronize_session=False
                )
                # Then delete the Records entry and create ChangeLog
                session.add(
                    ChangeLog(
                        key="bibcode:%s" % bibcode,
                        type="deleted",
                        oldvalue=serializer.dumps(r.toJSON()),
                    )
                )
                session.delete(r)
            ...
            if r is not None:
                ...
                if t is None:
                    ...
                    while t is not None:
                        ...
                        t = (
                            session.query(IdentifierMapping)
                            .filter_by(key=target)
                            .first()
                        )

                session.add(
                    ChangeLog(
                        key="bibcode:%s" % new_bibcode,
                        type="renamed",
                        oldvalue=r.bibcode,
                        permanent=True,
                    )
                ...
                self.logger.error(
                    "Rename operation, bibcode doesnt exist: old=%s, new=%s",
                    old_bibcode,
                    new_bibcode,
                )
    ...
    def get_msg_type(self, msg):
        ...
        if isinstance(msg, OrcidClaims):
            ...
            raise exceptions.IgnorableException(
                "Unkwnown type {0} submitted for update".format(repr(msg))
            )
    ...
    def index_solr(
        ...
        """
        self.logger.debug(
            "Updating solr: num_docs=%s solr_urls=%s", len(solr_docs), solr_urls
        )
        ...
        if len(errs) == 0:
            ...
            for doc, checksum in zip(solr_docs, solr_docs_checksum):
                try:
                    ...
                    solr_updater.update_solr(
                        [doc], solr_urls, ignore_errors=False, commit=commit
                    )
                    if update_processed:
                        self.mark_processed(
                            (doc["bibcode"],),
                    ...
                    if "body" in str(
                        ...
                        try:
                            solr_updater.update_solr(
                                [tmp_doc], solr_urls, ignore_errors=False, commit=commit
                            )
                            if update_processed:
                                self.mark_processed(
                                    (doc["bibcode"],),
            ...
            if failed_bibcodes and update_processed:
                self.mark_processed(
                    failed_bibcodes, "solr", checksums=None, status="solr-failed"
                )
    ...
    def mark_processed(self, bibcodes, type, checksums=None, status=None):
        ...
        if status:
            updt["status"] = status
        self.logger.debug(
            "Marking docs as processed: now=%s, num bibcodes=%s", now, len(bibcodes)
        )
        if type == "solr":
            ...
            raise ValueError(
                "invalid type value of %s passed, must be solr, metrics or links" % type
            )
        ...
        for bibcode, checksum in zip(bibcodes, checksums):
            ...
            with self.session_scope() as session:
                session.query(Records).filter_by(bibcode=bibcode).update(
                    updt, synchronize_session=False
                )
        session.commit()
    ...
    def get_metrics(self, bibcode):
        ...
        with self.metrics_session_scope() as session:
            x = (
                session.query(MetricsModel)
                .filter(MetricsModel.bibcode == bibcode)
                .first()
            )
            if x:
                return x.toJSON()
    ...
    def metrics_session_scope(self):
        ...
        if self._metrics_session is None:
            raise Exception(
                "DB not initialized properly, check: METRICS_SQLALCHEMY_URL"
            )
        ...
        with self.metrics_session_scope() as session:
            if len(batch):
                ...
                try:
                    ...
                    trans.rollback()
                    self.logger.error(
                        "Metrics insert batch failed, will upsert one by one %s recs",
                        len(batch),
                    )
                    failed_bibcodes = []
                    for x, checksum in zip(batch, batch_checksum):
                        try:
                            ...
                            self.logger.exception(
                                "Failed posting individual bibcode %s to metrics",
                                failed_bibcode,
                            )
                            failed_bibcodes.append(failed_bibcode)
                    if failed_bibcodes and update_processed:
    ...
    def index_datalinks(self, links_data, links_data_checksum, update_processed=True):
        ...
        if len(links_data):
            ...
            if r.status_code == 200:
                self.logger.info(
                    "sent %s datalinks to %s including %s",
                    len(links_data),
                    links_url,
                    links_data[0],
                )
                if update_processed:
                    self.mark_processed(
                        bibcodes,
                        "links",
                        checksums=links_data_checksum,
                        status="success",
                    )
            else:
                # recover from errors by issuing put requests one by one
                self.logger.error(
                    "error sending links to %s, error = %s", links_url, r.text
                )
                failed_bibcodes = []
                for data, checksum in zip(links_data, links_data_checksum):
                    ...
                    if r.status_code == 200:
                        self.logger.info(
                            "sent 1 datalinks to %s for bibcode %s",
                            links_url,
                            data.get("bibcode"),
                        )
                        if update_processed:
                        ...
                        self.logger.error(
                            "error sending individual links to %s for bibcode %s, error = %s",
                            links_url,
                            data.get("bibcode"),
                            r.text,
                        )
    ...
    def request_aff_augment(self, bibcode, data=None):
        ...
        if data is None:
            ...
            if rec is None:
                self.logger.warning(
                    "request_aff_augment called but no data at all for bibcode {}".format(
                        bibcode
                    )
                )
                return
            ...
            if bib_data is None:
                self.logger.warning(
                    "request_aff_augment called but no bib data for bibcode {}".format(
                        bibcode
                    )
                )
                return
        ...
        if data and data["aff"]:
            ...
            self.logger.debug(
                "sent augment affiliation request for bibcode {}".format(bibcode)
            )
        else:
            self.logger.debug(
                "request_aff_augment called but bibcode {} has no aff data".format(
                    bibcode
                )
            )
    ...
    def prepare_bibcode(self, bibcode):
        ...
        if rec is None:
            self.logger.warning(
                "request_classifier called but no data at all for bibcode {}".format(
                    bibcode
                )
            )
            return
        ...
        if bib_data is None:
            self.logger.warning(
                "request_classifier called but no bib data for bibcode {}".format(
                    bibcode
                )
            )
            return
    ...
    def request_classify(
        ...
        """
        self.logger.info(
            "request_classify called with bibcode={}, filename={}, mode={}, batch_size={}, data={}, validate={}".format(
                bibcode, filename, mode, batch_size, data, check_boolean
            )
        )

        if not self._config.get("OUTPUT_TASKNAME_CLASSIFIER"):
            self.logger.warning(
                "request_classifier called but no classifier taskname in config"
            )
            return
        if not self._config.get("OUTPUT_CELERY_BROKER_CLASSIFIER"):
            self.logger.warning(
                "request_classifier called but no classifier broker in config"
            )
            return
        ...
        if bibcode is not None and mode == "auto":
            ...
            if data and data.get("title"):
                ...
                self.logger.debug(
                    "sent classifier request for bibcode {}".format(bibcode)
                )
            else:
                self.logger.debug(
                    "request_classifier called but bibcode {} has no title data".format(
                        bibcode
                    )
                )
        if filename is not None and mode == "manual":
            ...
            batch_list = []
            self.logger.info(
                "request_classifier called with filename {}".format(filename)
            )
            ...
            while batch_idx < len(bibcodes):
                ...
                if len(batch_list) > 0:
                    ...
                    if check_boolean is True:
                        ...
                        self.forward_message(message, pipeline="classifier")
                        self.logger.debug(
                            "sent classifier request for batch {}".format(batch_idx)
                        )

                batch_idx += batch_size
                batch_list = []

    def _populate_boost_request_from_record(
        self,
        rec,
        metrics,
        classifications,
        ...
        message = {
            ...
            "bib_data": bib_data.decode("utf-8")
            if isinstance(bib_data, bytes)
            else bib_data,
    ...
    def generate_boost_request_message(self, bibcode, run_id=None, output_path=None):
        ...
        try:
            ...
            message = self._populate_boost_request_from_record(
                rec, metrics, classifications, run_id, output_path, None
            )

        except Exception as e:
            self.logger.error(
                "Error retrieving record data for bibcode %s: %s", bibcode, e
            )
        ...
        try:
            ...
            self.logger.info(
                "Sent boost request for bibcode %s to Boost Pipeline", bibcode
            )
            return True

        except Exception as e:
            self.logger.exception(
                "Error sending boost request for bibcode %s: %s", bibcode, e
            )
            return False
    ...
    def generate_links_for_resolver(self, record):
        ...
        if nonbib_links:
            ...
            resolver_record = {"bibcode": bibcode, "data_links_rows": nonbib_links}
        else:
                ...
                try:
                    ...
                    self.logger.error(
                        "invalid value in bib data, bibcode = {}, type = {}, value = {}".format(
                            bibcode, type(bib_links_record), bib_links_record
                        )
                    )
    ...
    def should_include_in_sitemap(self, record):
        ...
        if not has_bib_data or not bibcode:
            self.logger.debug(
                "Excluding %s from sitemap: No bibcode or has_bib_data is False",
                bibcode,
            )
            return False
        ...
        if status in ["solr-failed", "retrying"]:
            self.logger.warning(
                "Excluding %s from sitemap: SOLR failed or retrying (status: %s)",
                bibcode,
                status,
            )
            return False
        ...
        if not solr_processed:
            self.logger.debug(
                "Including %s in sitemap: Has bib_data, pending SOLR processing",
                bibcode,
            )
            return True
        ...
        if bib_data_updated and solr_processed:
            ...
            if processing_lag > timedelta(days=5):
                self.logger.debug(
                    "SOLR processing might be stale for %s by %s (bib_data_updated: %s, solr_processed: %s) - keeping in sitemap",
                    bibcode,
                    processing_lag,
                    bib_data_updated,
                    solr_processed,
        ...
        self.logger.debug(
            "Including %s in sitemap: Valid (status: %s, solr_processed: %s)",
            bibcode,
            status,
            solr_processed,
        )
    ...
    def get_sitemap_info_bulk(self, bibcodes, session):
        ...
        sitemap_infos = (
            session.query(SitemapInfo).filter(SitemapInfo.bibcode.in_(bibcodes)).all()
        )
    ...
    def get_current_sitemap_state(self, session):
        ...
        if results:
            ...
            return {"filename": filename, "count": count, "index": index}

        # No files exist yet, start with file 1
        return {"filename": "sitemap_bib_1.xml", "count": 0, "index": 1}
        ...
        for bibcode in bibcodes:
            try:
                ...
                if not self.should_include_in_sitemap(record):
                    self.logger.debug(
                        "Skipping %s: does not meet sitemap inclusion criteria", bibcode
                    )
                    failed_count += 1
                    continue
                ...
                if sitemap_info is None:
                    ...
                    sitemap_records.append(
                        (sitemap_record["record_id"], sitemap_record["bibcode"])
                    )
                    current_count += 1

                else:
                    # Existing sitemap record - update it
                    sitemap_record["filename_lastmoddate"] = sitemap_info.get(
                        "filename_lastmoddate", None
                    )
                    sitemap_record["sitemap_filename"] = sitemap_info.get(
                        "sitemap_filename", None
                    )
                    ...
                    if action == "force-update":
                        ...
                        if file_modified is None or (
                            file_modified
                            and bib_data_updated
                            and bib_data_updated > file_modified
                        ):
                            sitemap_record["update_flag"] = True
                ...
                successful_count += 1
                self.logger.debug(
                    "Successfully processed sitemap for bibcode: %s", bibcode
                )

            except Exception as e:
                failed_count += 1
                self.logger.error(
                    "Failed to process sitemap for bibcode %s: %s", bibcode, str(e)
                )
                continue
    ...
    def backup_sitemap_files(self, directory):
        ...
        tmpdir = "/app/logs/tmp/sitemap_{}_{}_{}-{}/".format(
    ...
    def _execute_remove_action(self, session, bibcodes_to_remove):
        ...
        files_to_delete = set(
            file for file in affected_files if file not in file_counts_after
        )
        files_to_update = affected_files - files_to_delete

        self.logger.info(
            "Removed %d bibcodes from %d files, %d files now empty, %d files pending regeneration flag",
            removed_count,
            len(affected_files),
            len(files_to_delete),
            len(files_to_update),
    ...
    def delete_sitemap_files(self, files_to_delete, sitemap_dir):
        ...
        self.logger.info(
            "Deleted %d empty sitemap files across all sites", deleted_count
        )

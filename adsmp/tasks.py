...
from sqlalchemy import MetaData, Table, create_engine, exc, insert
from sqlalchemy.orm import load_only, sessionmaker

from adsmp import app as app_module
from adsmp import solr_updater, templates
from adsmp.models import Records, SitemapInfo

# ============================= INITIALIZATION ==================================== #

proj_home = os.path.realpath(os.path.join(os.path.dirname(__file__), "../"))
app = app_module.ADSMasterPipelineCelery(
    "master-pipeline",
    proj_home=proj_home,
...
app.conf.CELERY_QUEUES = (
    ...
    Queue(
        "index-data-links-resolver",
        app.exchange,
        routing_key="index-data-links-resolver",
    ),
    Queue("manage-sitemap", app.exchange, routing_key="manage-sitemap"),
    Queue(
        "generate-single-sitemap", app.exchange, routing_key="generate-single-sitemap"
    ),
    ...
    if status == "active":
        ...
        if type == "nonbib_records":
            for m in msg.nonbib_records:
                m = Msg(
                    m, None, None
                )  # m is a raw protobuf, TODO: return proper instance from .nonbib_records
            ...
            for m in msg.metrics_records:
                m = Msg(m, None, None)
                bibcodes.append(m.bibcode)
                record = app.update_storage(
                    m.bibcode, "metrics", m.toJSON(including_default_value_fields=True)
                )
                if record:
                    logger.debug("Saved record from list: %s", record)
        elif type == "augment":
            ...
            record = app.update_storage(
                msg.bibcode, "augment", msg.toJSON(including_default_value_fields=True)
            )
            if record:
            ...
            bibcodes.append(msg.bibcode)
            logger.debug(
                f"message to JSON: {msg.toJSON(including_default_value_fields=True)}"
            )
            ...
            if type != "boost" and not app._config.get("TESTING_MODE", False):
                try:
                    ...
                    app.logger.exception(
                        "Error generating boost request message for bibcode %s: %s",
                        msg.bibcode,
                        e,
                    )
    else:
    ...
    if status == "deleted":
        if type == "metadata":
            ...
            for (
                ...
                msg.nonbib_records
            ):  # TODO: this is very ugly, we are repeating ourselves...
                bibcodes.append(m.bibcode)
...
def _generate_boost_request(msg, msg_type):
    # Send payload to Boost pipeline
    if msg_type not in app._config.get(
        "IGNORED_BOOST_PAYLOAD_TYPES", ["boost"]
    ) and not app._config.get("TESTING_MODE", False):
        try:
            task_boost_request.apply_async(args=(msg.bibcode,))
        except Exception as e:
            app.logger.exception(
        ...
        app.logger.debug(
            "Message for bibcode %s has type: %s, Skipping.".format(
                msg.bibcode, msg_type
            )
        )
...
def task_update_scixid(bibcodes, flag):
    ...
    """
    logger.info(
        "Starting task_update_scixid with flag: %s for %s bibcodes", flag, len(bibcodes)
    )
    if flag not in ["update", "force", "reset"]:
        logger.error(
            'task_update_scixid flag can only have the values "update" or "force"'
        )
        return
    ...
    for bibcode in bibcodes:
        ...
        with app.session_scope() as session:
                ...
                if not r.scix_id:
                    if r.bib_data:
                        ...
                        try:
                            session.commit()
                            logger.debug(
                                "Bibcode %s has been assigned a new scix id: %s",
                                bibcode,
                                r.scix_id,
                            )
                        except exc.IntegrityError:
                            session.rollback()
                            app.log_scix_id_collision(bibcode, attempted_scix_id)
                    else:
                        r.scix_id = None
                        session.commit()
                        logger.debug(
                            "Bibcode %s has no bib_data, scix_id is set to None",
                            bibcode,
                        )
                else:
                    logger.debug(
                        "Bibcode %s already has a scix id assigned: %s",
                        bibcode,
                        r.scix_id,
                    )
            ...
            if flag == "force":
                if r.bib_data:
                    ...
                    try:
                        session.commit()
                        logger.debug(
                            "Bibcode %s scix_id changed from %s to %s",
                            bibcode,
                            old_id,
                            r.scix_id,
                        )
                    ...
                    session.commit()
                    logger.debug(
                        "Bibcode %s has no bib_data, scix_id is set to None", bibcode
                    )
            ...
            if flag == "reset":
                ...
                session.commit()
                logger.debug(
                    "Bibcode %s scix_id reset from %s to None", bibcode, old_id
                )
...
def task_index_metrics(
    metrics_records, metrics_records_checksum, update_processed=True
):
...
def task_index_data_links_resolver(
    links_data_records, links_data_records_checksum, update_processed=True
):
    app.index_datalinks(
        links_data_records,
    ...
    for bibcode in bibcodes:
        ...
        if is_complete or (force is True and bib_data_updated):
            if force is False and all(
                ...
                logger.debug(
                    "Nothing to do for %s, it was already indexed/processed", bibcode
                )
                continue
            if force:
                logger.debug(
            ...
            if update_solr:
                ...
                if (
                    "bibcode" in solr_payload
                    and solr_payload["bibcode"] not in solr_payload["identifier"]
                ):
                ...
                if ignore_checksums or r.get("solr_checksum", None) != solr_checksum:
                    ...
                    logger.debug(
                        "Checksum identical, skipping solr update for: %s", bibcode
                    )
            ...
            if update_metrics:
                ...
                if (metrics_payload and ignore_checksums) or (
                    metrics_payload
            ...
            if update_links and links_url:
                ...
                if datalinks_payload:
                    ...
                    if (
                        ignore_checksums
                        or r.get("datalinks_checksum", None) != datalinks_checksum
                    ):
    ...
    while True:
        with app.session_scope() as session:
            ...
            if bibcodes_to_remove:
                logger.info(
                    "Batch %d: removing %d invalid records (processed %d total)",
                    batch_count,
                    len(bibcodes_to_remove),
                    total_processed,
                )
                ...
                (
                    ...
                    files_to_delete,
                    files_to_update,
                ) = app._execute_remove_action(session, bibcodes_to_remove)
                ...
                logger.debug(
                    "Batch %d: no invalid records found (processed %d total)",
                    batch_count,
                    total_processed,
                )
...
def task_manage_sitemap(bibcodes, action):
    ...
    if action == "delete-table":
        ...
        logger.info(
            "Removing %d bibcodes from sitemaps using batch processing (batch size: %d)",
            total_bibcodes,
            batch_size,
        )
        ...
        with app.session_scope() as session:
            ...
            for batch_num, batch in enumerate(chunked(bibcodes, batch_size), 1):
                logger.info(
                    "Processing remove batch %d/%d (%d bibcodes)",
                    batch_num,
                    total_batches,
                    len(batch),
                )
                ...
                (
                    ...
                    files_to_delete,
                    files_to_update,
                ) = app._execute_remove_action(session, batch)
                ...
                logger.info(
                    "Batch %d completed: %d bibcodes removed, %d files marked for deletion",
                    batch_num,
                    removed_count,
                    len(files_to_delete),
                )
        ...
        logger.info(
            "Remove operation completed: %d total bibcodes removed, %d empty files deleted, %d files flagged",
            total_removed,
            len(all_files_to_delete),
            flagged,
        )
        return
        ...
        with app.session_scope() as session:
            ...
            if existing_sitemap_count > 0:
                logger.warning(
                    f"SitemapInfo table already has {existing_sitemap_count} records"
                )
                logger.warning(
                    'Use "force-update" action if you want to update existing sitemap records'
                )
                return
            ...
            logger.info(
                "Using keyset pagination for efficient processing of large datasets"
            )
            ...
            while True:
                ...
                for record in records_batch:
                    try:
                        ...
                        if not app.should_include_in_sitemap(record_dict):
                            logger.debug(
                                "Skipping %s in bootstrap: does not meet sitemap inclusion criteria",
                                record.bibcode,
                            )
                            failed_count += 1
                            continue
                        ...
                        logger.error(
                            "Failed to prepare record %s: %s", record.bibcode, str(e)
                        )
                        failed_count += 1
                        continue
                ...
                # Log progress
                if (
                    processed % (batch_size * 5) == 0 or not records_batch
                ):  # Log every 5 batches
                    logger.info(
                        "Bootstrapped %d records - %d successful, %d failed",
                        processed,
                        successful_count,
                        failed_count,
                    )
            ...
            logger.info(
                "Bootstrap completed: %d successful, %d failed out of %d total records",
                successful_count,
                failed_count,
                processed,
            )
        ...
        with app.session_scope() as session:
            ...
            for batch_num, batch in enumerate(chunked(bibcodes, batch_size), 1):
                ...
                logger.info(
                    "Batch %d completed: %d successful (%d new, %d updated), %d failed",
                    batch_num,
                    batch_stats["successful"],
                    batch_stats["new_records"],
                    batch_stats["updated_records"],
                    batch_stats["failed"],
                )

        logger.debug(
            "Sitemap population completed: %d successful, %d failed out of %d total bibcodes",
            overall_successful_count,
            overall_failed_count,
            total_bibcodes,
        )
        logger.debug(
            "Records breakdown: %d new, %d updated (total: %d)",
            overall_new_count,
            overall_updated_count,
            ...
            try:
                ...
                if not needs_update:
                    ...
                    try:
                        ...
                        needs_update = (
                            existing_content.strip() != expected_content.strip()
                        )
                    except Exception:
                ...
                if needs_update:
                    ...
                    logger.info(
                        "Updated robots.txt for site %s at %s",
                    ...
                    logger.debug(
                        "robots.txt for site %s is up to date",
                        site_config.get("name", site_key),
                    )

            except Exception as e:
                logger.error(
                    "Failed to update robots.txt for site %s: %s", site_key, str(e)
                )
                continue
...
def update_sitemap_index():
    ...
    try:
        ...
        with app.session_scope() as session:
            ...
            sitemap_filenames = (
                [filename[0] for filename in sitemap_files] if sitemap_files else []
            )

            if not sitemap_filenames:
                logger.info(
                    "No sitemap files found in database - generating empty index files"
                )
            ...
            for site_key, site_config in sites_config.items():
                try:
                    ...
                    if os.path.exists(static_template_path):
                        ...
                        entry = templates.format_sitemap_entry(
                            sitemap_base_url, "sitemap_static.xml", lastmod_date
                        )
                    ...
                    for filename in sitemap_filenames:
                        ...
                        if os.path.exists(sitemap_filepath):
                            ...
                            entry = templates.format_sitemap_entry(
                                sitemap_base_url, filename, lastmod_date
                            )
                            sitemap_entries.append(entry)

                    # Always generate sitemap index content, even if empty
                    index_content = templates.render_sitemap_index(
                    ...
                    if sitemap_entries:
                        logger.info(
                            "Generated sitemap index for site %s with %d entries at %s",
                            site_config.get("name", site_key),
                            len(sitemap_entries),
                            index_filepath,
                        )
                        ...
                        logger.info(
                            "Generated empty sitemap index for site %s at %s",
                            site_config.get("name", site_key),
                            index_filepath,
                        )
                    updated_sites += 1
                    ...
                    logger.error(
                        "Failed to generate sitemap index for site %s: %s",
                        site_key,
                        str(e),
                    )
                    continue
        ...
        logger.info(
            "Sitemap index generation completed: %d sites updated", updated_sites
        )
        return True
...
def task_generate_single_sitemap(sitemap_filename, record_ids):
    ...
    logger.info(
        "Generating sitemap file: %s (%d records)", sitemap_filename, len(record_ids)
    )
    ...
    try:
        ...
        with app.session_scope() as session:
            ...
            file_records = (
                session.query(SitemapInfo).filter(SitemapInfo.id.in_(record_ids)).all()
            )
            ...
            for site_key, site_config in sites_config.items():
                try:
                    ...
                    abs_url_pattern = site_config.get(
                    ...
                    for info in file_records:
                        ...
                        lastmod_date = (
                            info.bib_data_updated.date()
                            if info.bib_data_updated
                            else adsputils.get_date().date()
                        )
                        # Use site-specific URL pattern
                        # Ex: https://ui.adsabs.harvard.edu/abs/2023ApJ...123..456A/abstract
                        url_entry = templates.format_url_entry(
                            info.bibcode, lastmod_date, abs_url_pattern
                        )
                    ...
                    sitemap_content = templates.render_sitemap_file(
                        "".join(url_entries)
                    )
                    ...
                    logger.debug(
                        "Successfully generated %s for site %s with %d records",
                        site_filepath,
                        site_config.get("name", site_key),
                        len(url_entries),
                    )
                    ...
                    logger.error(
                        "Failed to generate sitemap file %s for site %s: %s",
                        sitemap_filename,
                        site_key,
                        str(e),
                    )
            ...
            if successful_sites > 0:
                ...
                session.commit()
                logger.info(
                    "Completed sitemap file: %s (%d records, %d sites)",
                    sitemap_filename,
                    len(file_records),
                    successful_sites,
                )
...
def task_generate_sitemap_index(self, retry_count=0):
    ...
    try:
        logger.info(
            "Generating sitemap index files (attempt %d/%d)",
            retry_count + 1,
            max_retries,
        )
        ...
        with app.session_scope() as session:
            ...
            if pending_count > 0:
                if retry_count >= max_retries - 1:
                    logger.warning(
                        "Reached max retries (%d) with %d records still pending - generating index anyway",
                        max_retries,
                        pending_count,
                    )
                    ...
                    logger.info(
                        "Found %d records still pending file generation - will retry in %ds (attempt %d/%d)",
                        pending_count,
                        retry_delay,
                        retry_count + 1,
                        max_retries,
                    ...
                    raise self.retry(
                        countdown=retry_delay, kwargs={"retry_count": retry_count + 1}
                    )
...
def task_update_sitemap_files(previous_result=None):
    ...
    try:
        ...
        if not success:
            logger.warning(
                "robots.txt update failed, but continuing with sitemap generation"
            )
        ...
        with app.session_scope() as session:
            files_needing_update_subquery = session.query(
                SitemapInfo.sitemap_filename.distinct()
            ).filter(SitemapInfo.update_flag == True)
            ...
            if not all_records:
                ...
                logger.info(
                    "Regenerating sitemap index to reflect current database state"
                )
                success = update_sitemap_index()
                if success:
            ...
            logger.info(
                "Found %d records to process in sitemap files", len(all_records)
            )
        ...
        logger.info(
            "Starting file generation for %d sitemap files (%d total records)",
            len(files_dict),
            len(all_records),
        )
        ...
        for sitemap_filename, record_ids in files_dict.items():
            ...
            logger.debug(
                "Spawned task for %s with %d records", sitemap_filename, len(record_ids)
            )
        ...
        if files_dict:
            ...
            logger.info(
                "Index generation task submitted (will wait for files to complete)"
            )
...
def task_boost_request(bibcodes):
    ...
    logger.info(
        "Boost requests for %s bibcode(s) sent to boost pipeline", len(bibcodes)
    )
